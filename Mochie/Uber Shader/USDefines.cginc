...
#define CUT_OR_TRANS_DEFINED (defined(CUTOUT) || defined(TRANSPARENT))
...
int _StaticLightDirToggle, _ClampAdditive;
int _Shadows, _EnableShadowRamp, _RTSelfShadow;
int _Reflections, _ReflCubeFallback, _UseReflCube;
int _Specular, _SharpSpecular, _SpecularStyle, _AnisoLerp;
int _MatcapToggle, _MatcapBlending;
...
float _DirectCont, _IndirectCont, _RTDirectCont, _RTIndirectCont, _VLightCont, _AdditiveMax;
float _ShadowStr, _RampWidth0, _RampWidth1, _RampWeight;
float _ReflectionStr;
float _SpecStr, _AnisoLayerStr, _AnisoAngleX, _AnisoAngleY, _AnisoLayerX, _AnisoLayerY, _ProbeContrib;
float _MatcapStr;
float _RimStr, _RimWidth, _RimEdge;
float _RoughLightness, _RoughIntensity, _RoughContrast;
float _SmoothShading, _DisneyDiffuse;
...
struct g2f {
	...
	float3 cameraPos : TEXCOORD7;
	float3 objPos : TEXCOORD8;
	float3 bCoords : TEXCOORD9;
	float WFStr : TEXCOORD10;
	uint instID : TEXCOORD11;
	bool isVLight : TEXCOORD12;
